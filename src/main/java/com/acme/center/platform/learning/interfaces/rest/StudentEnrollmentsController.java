...
public class StudentEnrollmentsController {
    ...
    public ResponseEntity<List<EnrollmentResource>> getEnrollmentsForStudentWithStudentRecordId(@PathVariable String studentRecordId) {
        ...
        var existsByAcmeStudentRecordIdQuery = new ExistsByAcmeStudentRecordIdQuery(acmeStudentRecordId);
        if (studentQueryService.handle(existsByAcmeStudentRecordIdQuery)) return ResponseEntity.notFound().build();

...
public class CourseQueryServiceImpl implements CourseQueryService {
    ...
    public Optional<LearningPathItem> handle(GetLearningPathItemByCourseIdAndTutorialIdQuery query) {
        if(!courseRepository.existsById(query.courseId())) throw new IllegalArgumentException("Course not found with ID: " + query.courseId());
        return courseRepository.findById(query.courseId())
                .map(course -> course.getLearningPath().getLearningPathItemWithTutorialId(query.tutorialId()));
